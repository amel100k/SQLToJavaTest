...
public class JAVAexamples2 {
	...
	try {
		...
		// gennemløber svaret
		while (res.next()) {
			String init = res.getString(1);
			String rytternavn = res.getString(2);
			System.out.println(init + "    " + rytternavn);
		}
		// pæn lukning
	...
	try {
		...
		// Laver sql-sætning og får den udført
		String sql = "select r.init, r.rytternavn from rytter r where r.rytternavn like '%" + inString + "%'";
		System.out.println("SQL-streng er "+ sql);
		ResultSet res=stmt.executeQuery(sql);
		//gennemløber svaret
		while (res.next()) {
